...
class CorrRandVariables2D(ABC):
    ...
    def __init__(self, rand_name1='Marginal1', rand_name2='Marginal2'):
        ...
        ----------
        rand_name1: str
            name of random number 1
        rand_name2: str
            name of random number 2
        """
    ...
    def calc_marginals(self):
        rv1 = self.gen_rand_num1()
        rv2 = self.gen_rand_num2()
        self.marginals = np.stack([rv1, rv2], axis=0)

    def calc_corr_rand(self, rho):
    ...
    def plot_joint_scatter(self, ax=None):
        if not hasattr(self, 'corr_rands'):
            print('correlated RVs not calculated yet')
            return
        if ax is None:
            fig, ax = plt.subplots(figsize=(8, 8))
...
class CorrNormSkewT2D(CorrRandVariables2D):
    ...
    def calc_marginals(self, eta, lam):
        rv1 = self.gen_rand_num1()
        rv2 = self.gen_rand_num2(eta=eta, lam=lam)
        self.marginals = np.stack([rv1, rv2], axis=0)
...
def main():
    ...
    for rho, ax in zip(rhos, axes.flatten()):
        corr_rv.calc_corr_rand(rho=rho)
